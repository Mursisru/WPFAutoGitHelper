...
namespace WpfAutoGitHelper.ViewModels
{
    public sealed partial class MainViewModel
    {
        ...
        public const int AutoAdvancedRunTabIndex = 3;

        /// <summary>Log tab index in Advanced TabControl (Auto tab at 3 when visible).</summary>
        public int AdvancedLogTabIndex => IsAutoAdvancedMode ? 9 : 8;
        ...
        private UiModeKind ResolveStartupUiMode()
        {
            if (!string.IsNullOrWhiteSpace(_settings.DefaultUiMode))
                return ParseUiMode(_settings.DefaultUiMode);

            if (!string.IsNullOrWhiteSpace(_settings.UiMode))
                return ParseUiMode(_settings.UiMode);

            return UiModeKind.Easy;
        }
        ...
        private void SetUiMode(UiModeKind mode)
        {
            ...
            _uiMode = mode;
            var modeText = UiModeToSettingsText(mode);
            _settings.UiMode = modeText;
            _settings.DefaultUiMode = modeText;
            OnPropertyChanged(nameof(IsEasyMode));
            OnPropertyChanged(nameof(DefaultUiModeIsEasy));
            OnPropertyChanged(nameof(DefaultUiModeIsAdvanced));
            OnPropertyChanged(nameof(DefaultUiModeIsAutoAdvanced));
            ...
            OnPropertyChanged(nameof(AdvancedLogTabIndex));

            if (mode == UiModeKind.Easy)
                SelectedTabIndex = 0;
            else
            {
                AdvancedTabIndex = mode == UiModeKind.AutoAdvanced ? AutoAdvancedRunTabIndex : 0;
                if (HasValidRepo)
                {
